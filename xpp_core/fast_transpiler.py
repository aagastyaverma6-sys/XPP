...
def _split_comment(line: str):
    """Split line into (code, comment) respecting quoted string literals."""
    ...
    while True:
        ...
        if m.group(2):
            ...
            return line[:comment_start].rstrip(), line[comment_start:]
        pos = m.end()
    return line, ""


def _convert_dots(s: str) -> str:
    """Convert attribute access like p.version or a.b.c outside quotes into dict index lookups."""
...
def transpile(src: str) -> str:
    ...
    for raw_line in src.splitlines():
        ...
        # Skip directive lines (e.g. RNM=ZITR or USE MODEL "...")
        if re.match(r'^(?:RNM\s*=\s*\w+|USE\s+MODEL\b)', stripped, re.I):
            continue

        comm_suffix = ("  " + comment) if comment else ""
        ...
        if m:
            ...
            out.append(" " * indent + f"def {name}({params}):" + comm_suffix)
            stack.append("fn")
            indent += 4
            continue
        m = _control["if"].match(stripped)
        if m:
            out.append(" " * indent + f"if {_expr(m.group(1))}:" + comm_suffix)
            stack.append("if")
            indent += 4
            continue
        ...
        if m:
            indent = max(0, indent - 4)
            out.append(" " * indent + f"elif {_expr(m.group(1))}:" + comm_suffix)
            indent += 4
            continue
        m = _control["else"].match(stripped)
        if m:
            indent = max(0, indent - 4)
            out.append(" " * indent + "else:" + comm_suffix)
            indent += 4
            continue
        m = _control["while"].match(stripped)
        if m:
            out.append(" " * indent + f"while {_expr(m.group(1))}:" + comm_suffix)
            stack.append("while")
            indent += 4
            continue
        ...
        if m:
            ...
            if step:
                step_e = _expr(step)
                out.append(" " * indent + f"for {var} in _xpp_loop({start_e}, {end_e}, {step_e}):" + comm_suffix)
            else:
                out.append(" " * indent + f"for {var} in range({start_e}, ({end_e})+1):" + comm_suffix)
            stack.append("loop")
            indent += 4
            continue
        ...
        if m:
            var, it = m.groups()
            out.append(" " * indent + f"for {var} in {_expr(it)}:" + comm_suffix)
            stack.append("loop")
            indent += 4
            continue
        m = _control["safe"].match(stripped)
        if m:
            out.append(" " * indent + "try:" + comm_suffix)
            stack.append("safe")
            indent += 4
            continue
        ...
        if m:
            exc = m.group(1)
            indent = max(0, indent - 4)
            out.append(" " * indent + (f"except Exception as {exc}:" if exc else "except Exception:") + comm_suffix)
            indent += 4
            continue
        m = _control["return"].match(stripped)
        if m:
            v = m.group(1)
            out.append(" " * indent + (f"return {_expr(v)}" if v else "return") + comm_suffix)
            continue
        if _control["break"].match(stripped):
            out.append(" " * indent + "break" + comm_suffix)
            continue
        if _control["continue"].match(stripped):
            out.append(" " * indent + "continue" + comm_suffix)
            continue
        m = _control["out"].match(stripped)
        if m:
            out.append(" " * indent + f"print({_expr(m.group(1))})" + comm_suffix)
            continue
        m = _control["push"].match(stripped)
        if m:
            val, lst = m.groups()
            out.append(" " * indent + f"{lst}.append({_expr(val)})" + comm_suffix)
            continue
        m = _control["assign"].match(stripped)
        if m:
            lhs, rhs = m.groups()
            lhs_converted = _convert_dots(lhs)
            out.append(" " * indent + f"{lhs_converted} = {_expr(rhs)}" + comm_suffix)
            continue
        out.append(" " * indent + _expr(stripped) + comm_suffix)
    return "\n".join(out) + ("\n" if out else "")
